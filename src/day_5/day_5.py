...
def part_one(use_test_input: bool):
    lines = __get_input_lines(use_test_input)

    stacks = __initialize_stacks(lines)
...
def __initialize_stacks(lines: List[str]) -> List[List]:
    number_of_stacks = __get_number_of_stacks(lines)
...
def __get_number_of_stacks(lines: List[str]) -> int:
    # find line with the numbers
    line_with_numbers = ''
    for line in lines:
        if line.startswith(' 1'):
    ...
    # this line ends with '{last number} \n', so just return those last 3 chars stripped
